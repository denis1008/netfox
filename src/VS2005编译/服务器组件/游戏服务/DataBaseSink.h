...
private:
	...
	//加载用户
	bool OnRequestLoadAndroid(const NTY_DataBaseEvent & DataBaseEvent, void * pDataBuffer, WORD wDataSize);
	//赠送鲜花
	bool OnRequestSendGift(const NTY_DataBaseEvent & DataBaseEvent, void * pDataBuffer, WORD wDataSize);
...
protected:
	...
	//记录赠送
	LONG SPRecordGiftGrant(DWORD dwSendUserID, DWORD dwRcvUserID, WORD wGiftID, DWORD dwClientIP);
};
