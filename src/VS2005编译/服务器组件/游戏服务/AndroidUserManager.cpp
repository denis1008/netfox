...
{
	//启动时间
	m_pITimerEngine->SetTimer(IDI_ANDROID_INOUT,TIME_ANDROID_INOUT*1000L,TIMER_REPEAT_TIMER,0);
	m_pITimerEngine->SetTimer(IDI_ANDROID_USER_PULSE,TIME_ANDROID_USER_PULSE*1000L,TIMER_REPEAT_TIMER,0);
...
bool __cdecl CAndroidUserManager::DeleteAndroidUserItem(DWORD dwAndroidID)
{
	...
	//关闭事件
	m_AttemperEvent.PostSocketCloseEvent(LOWORD(dwAndroidID),HIWORD(dwAndroidID),0L,0L);
...
IAndroidUserItem * __cdecl CAndroidUserManager::CreateAndroidUserItem(DWORD dwUserID, TCHAR szPassword[PASS_LEN])
{
	...
	//连接模拟
	m_AttemperEvent.PostSocketAcceptEvent(wAndroidIndex,wRoundID,0L);
	...
	//发送数据
	m_AttemperEvent.PostSocketReadEvent(wAndroidIndex,wRoundID,Command,&LogonByUserID,sizeof(LogonByUserID));
...
bool __cdecl CAndroidUserManager::SendDataToClient(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	for (INT_PTR i=0;i<m_AndroidUserItemStorage.GetCount();i++)
	{
		...
		try
		{
			bSuccess=pAndroidUserItem->OnSocketRead(wMainCmdID,wSubCmdID,pData,wDataSize);
		}
		catch (...) {}
...
bool __cdecl CAndroidUserManager::SendDataToServer(DWORD dwAndroidID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	//发送数据
	m_AttemperEvent.PostSocketReadEvent(LOWORD(dwAndroidID),HIWORD(dwAndroidID),Command,pData,wDataSize);
...
CAndroidUserItem * CAndroidUserManager::ActiveAndroidUserItem(DWORD dwUserID)
{
	...
	if (pAndroidUserItem==NULL)
	{
		...
		try
		{
			...
			pAndroidUserItem->m_pIAndroidUserManager=GET_MYSELF_INTERFACE(IAndroidUserManager);
		}
		catch (...) 
		{ 
			ASSERT(FALSE);
