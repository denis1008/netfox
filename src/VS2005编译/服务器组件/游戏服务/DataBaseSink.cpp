...
bool __cdecl CDataBaseSink::OnDataBaseRequest(const NTY_DataBaseEvent & DataBaseEvent, void * pDataBuffer, WORD wDataSize)
{
	...
	case DBR_GR_LOAD_ANDROID:			//加载用户
		{
			return OnRequestLoadAndroid(DataBaseEvent,pDataBuffer,wDataSize);
		}
	case DBR_GR_SEND_GIFT:				//赠送鲜花
		{
			return OnRequestSendGift(DataBaseEvent,pDataBuffer,wDataSize);
		}
	}
...
//加载用户
bool CDataBaseSink::OnRequestLoadAndroid(const NTY_DataBaseEvent & DataBaseEvent, void * pDataBuffer, WORD wDataSize)
{
	try
	{
		//变量定义
		...
		m_GameScoreDB->SetSPName("GSP_GR_LoadAndroidUser");
		...
		AndroidUser.lResultCode=m_GameScoreDB->GetReturnValue();

		//读取信息
		for (WORD i=0;i<CountArray(AndroidUser.dwAndroidID);i++)
		{
			//结束判断
			if (m_GameScoreDB->IsEndRecordset()==true) break;

			//读取数据
			DWORD dwAndroidID=0L;
			m_GameScoreDB->GetFieldValue(TEXT("UserID"),dwAndroidID);
			AndroidUser.dwAndroidID[AndroidUser.wAndroidCount++]=dwAndroidID;

			//移动记录
			m_GameScoreDB->MoveToNext();
		}
		...
		m_AttemperEvent.PostDataBaseEvent(DBR_GR_ANDROID_USER,DataBaseEvent.wIndex,DataBaseEvent.wRoundID,&AndroidUser,wPacketSize);
...
//赠送鲜花
bool CDataBaseSink::OnRequestSendGift(const NTY_DataBaseEvent & DataBaseEvent, void * pDataBuffer, WORD wDataSize)
{
	//效验参数
	...
	//参数转换
	DBR_GR_SendGift * pSendGift=(DBR_GR_SendGift *)pDataBuffer;
	...
	try
	{
		...
		//设置消息
		DBR_GR_SendGiftResult SendGiftResult;
		ZeroMemory( &SendGiftResult, sizeof( SendGiftResult ) );

		SendGiftResult.dwSendUserID = pSendGift->dwSendUserID;
		SendGiftResult.dwRcvUserID = pSendGift->dwRcvUserID;
		SendGiftResult.wGiftID = pSendGift->wGiftID;
		SendGiftResult.bSuccess = lRet == 0 ? true : false;

		//发送信息
		m_AttemperEvent.PostDataBaseEvent(DBR_GR_SEND_GIFT_RESULT,DataBaseEvent.wIndex,DataBaseEvent.wRoundID,&SendGiftResult,sizeof( SendGiftResult ));
...
LONG CDataBaseSink::SPRecordGiftGrant(DWORD dwSendUserID, DWORD dwRcvUserID, WORD wGiftID, DWORD dwClientIP)
{
	...
	//执行存储过程
	m_GameScoreDB->ClearAllParameters();
	m_GameScoreDB->SetSPName("GSP_GR_RecordGiftGrant");
	m_GameScoreDB->AddParamter(TEXT("RETURN_VALUE"),adParamReturnValue,adInteger,sizeof(long),_variant_t((long)0));
	m_GameScoreDB->AddParamter(TEXT("@dwSendUserID"),adParamInput,adInteger,sizeof(long),_variant_t((long)dwSendUserID));
	m_GameScoreDB->AddParamter(TEXT("@dwRcvUserID"),adParamInput,adInteger,sizeof(long),_variant_t((long)dwRcvUserID));
	m_GameScoreDB->AddParamter(TEXT("@strGift"),adParamInput,adVarChar,lstrlen(g_GiftInfo[wGiftID].szName),_variant_t(g_GiftInfo[wGiftID].szName));
	m_GameScoreDB->AddParamter(TEXT("@lGiftPay"),adParamInput,adInteger,sizeof(long),_variant_t((long)g_GiftInfo[wGiftID].lValue));
	m_GameScoreDB->AddParamter(TEXT("@wKindID"),adParamInput,adInteger,sizeof(long),_variant_t((long)m_pGameServiceAttrib->wKindID));
	m_GameScoreDB->AddParamter(TEXT("@wServerID"),adParamInput,adInteger,sizeof(long),_variant_t((long)m_pGameServiceOption->wServerID));
	m_GameScoreDB->AddParamter(TEXT("@strClientIP"),adParamInput,adVarChar,lstrlen(szClientIP),_variant_t(szClientIP));
	m_GameScoreDB->ExecuteCommand(true);
