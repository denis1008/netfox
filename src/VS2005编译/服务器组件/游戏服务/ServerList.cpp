...
CServerListInfo::~CServerListInfo()
{
	...
	nCount	= m_GameTypePtrArrayActive.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameTypePtrArrayActive[i];
	m_GameTypePtrArrayActive.RemoveAll();

	nCount	= m_GameTypePtrArrayBuffer.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameTypePtrArrayBuffer[i];
	m_GameTypePtrArrayBuffer.RemoveAll();

	nCount	= m_GameKindPtrArrayActive.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameKindPtrArrayActive[i];
	m_GameKindPtrArrayActive.RemoveAll();

	nCount	= m_GameKindPtrArrayBuffer.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameKindPtrArrayBuffer[i];
	m_GameKindPtrArrayBuffer.RemoveAll();

	nCount	= m_GameServerPtrArrayActive.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameServerPtrArrayActive[i];
	m_GameServerPtrArrayActive.RemoveAll();

	nCount	= m_GameServerPtrArrayBuffer.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameServerPtrArrayBuffer[i];
	m_GameServerPtrArrayBuffer.RemoveAll();

	nCount	= m_GameStationPtrArrayActive.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameStationPtrArrayActive[i];
	m_GameStationPtrArrayActive.RemoveAll();

	nCount	= m_GameStationPtrArrayBuffer.GetCount();
	for(i=0; i<nCount; i++)
		delete	m_GameStationPtrArrayBuffer[i];
