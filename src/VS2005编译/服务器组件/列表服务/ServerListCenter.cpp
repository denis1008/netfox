...
bool __cdecl CServerListCenter::InsertGameType(tagGameType* pGameType)
{
	...
	//查找现存
	tagGameTypeItem*	pGameTypeItem	= NULL;
	if(m_TypeItemMap.Lookup(pGameType->wTypeID, pGameTypeItem)==TRUE)
		return false;
...
bool __cdecl CServerListCenter::InsertGameKind(tagGameKind* pGameKind)
{
	...
	//查找现存
	tagGameKindItem*	pGameKindItem	= NULL;
	if(m_KindItemMap.Lookup(pGameKind->wKindID,pGameKindItem)==TRUE)
		return	false;
...
bool __cdecl CServerListCenter::InsertGameServer(tagGameServer* pGameServer)
{
	...
	if(m_ServerItemMap.Lookup(dwKey,pGameServerItem)==TRUE)
		return	false;
	...
	try
	{
		...
		if(m_pIEventService!=NULL) 
		{
			...
			_snprintf(szBuffer, sizeof(szBuffer), TEXT("【%s  KindID：%ld StationID：%ld  ServerID：%ld】注册成功"), pGameServer->szServerName, pGameServer->wKindID, pGameServer->wStationID, pGameServer->wServerID);
...
void CServerListCenter::LoadGameStation(IDataBase * pIDataBase)
{
	...
	while (pIDataBase->IsEndRecordset()==false)
	{
		...
		pIDataBase->GetFieldValue(TEXT("NodeID"),GameStationRead.wStationID);
		pIDataBase->GetFieldValue(TEXT("NodeName"),GameStationRead.szStationName,sizeof(GameStationRead.szStationName));
		pIDataBase->MoveToNext();
