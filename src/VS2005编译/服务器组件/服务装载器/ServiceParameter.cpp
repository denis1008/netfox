...
void CServiceParameter::UpdateOptionParameter()
{
	...
	//类型标识
	if(m_OptionParameter.GameServiceOption.wKindID==0)
		SetDlgItemText(IDC_OPTION_KIND_ID, TEXT(""));
	else
		SetDlgItemInt(IDC_OPTION_KIND_ID, m_OptionParameter.GameServiceOption.wKindID, FALSE);

	//房间标识
	if(m_OptionParameter.GameServiceOption.wServerID==0)
		SetDlgItemText(IDC_SERVER_ID, TEXT(""));
	else
		SetDlgItemInt(IDC_SERVER_ID, m_OptionParameter.GameServiceOption.wServerID, FALSE);

	//站点标识
	if(m_OptionParameter.GameServiceOption.wStationID==0)
		SetDlgItemText(IDC_STATION_ID, TEXT(""));
	else
		SetDlgItemInt(IDC_STATION_ID, m_OptionParameter.GameServiceOption.wStationID, FALSE);

	//桌子数目
	if(m_OptionParameter.GameServiceOption.wTableCount==0)
		SetDlgItemText(IDC_TABLE_COUNT, TEXT(""));
	else
		SetDlgItemInt(IDC_TABLE_COUNT, m_OptionParameter.GameServiceOption.wTableCount, FALSE);

	//单元积分
	if(m_OptionParameter.GameServiceOption.lCellScore==0)
		SetDlgItemText(IDC_CELL_SCORE, TEXT(""));
	else
		SetDlgItemInt(IDC_CELL_SCORE, m_OptionParameter.GameServiceOption.lCellScore, FALSE);

	//下限积分
	if(m_OptionParameter.GameServiceOption.lLessScore==0)
		SetDlgItemText(IDC_LESS_SCORE, TEXT(""));
	else
		SetDlgItemInt(IDC_LESS_SCORE, m_OptionParameter.GameServiceOption.lLessScore, FALSE);

	//上限积分
	if(m_OptionParameter.GameServiceOption.lRestrictScore==0)
		SetDlgItemText(IDC_RESTRICT_SCORE, TEXT(""));
	else
		SetDlgItemInt(IDC_RESTRICT_SCORE, m_OptionParameter.GameServiceOption.lRestrictScore, FALSE);
	...
	if(lMatchDraw>0)
		SetDlgItemInt(IDC_MATCH_DRAW, lMatchDraw, FALSE);

	//最大连接
	if(m_OptionParameter.GameServiceOption.wMaxConnect==0)
		SetDlgItemText(IDC_MAX_CONNECT, TEXT(""));
	else
		SetDlgItemInt(IDC_MAX_CONNECT, m_OptionParameter.GameServiceOption.wMaxConnect, FALSE);
	...
	if(m_OptionParameter.GameServiceOption.szGameRoomName[0]==0) 
	{
		...
		SetDlgItemText(IDC_SERVER_NAME, m_OptionParameter.GameServiceOption.szGameRoomName);

	//视频地址
	((CIPAddressCtrl*)GetDlgItem(IDC_VIDEO_ADDR))->SetAddress(m_OptionParameter.GameServiceOption.dwVideoServer);

	//监听端口
	if(m_OptionParameter.GameServiceOption.wServerPort==0)
		SetDlgItemText(IDC_PORT, TEXT(""));
	else
		SetDlgItemInt(IDC_PORT, m_OptionParameter.GameServiceOption.wServerPort, FALSE);
	...
	//数据库名字
	if(m_OptionParameter.szDataBaseName[0]!=0)
		SetDlgItemText(IDC_DATABASE_NAME, m_OptionParameter.szDataBaseName);
	else
		SetDlgItemText(IDC_DATABASE_NAME, m_GameServiceAttrib.szDataBaseName);
