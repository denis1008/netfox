...
WORD CXOREncrypt::EncryptData(LPCTSTR pszSrcData, LPTSTR pszEncrypData, WORD wSize)
{
	...
	for(WORD i=1; i<CountArray(szRandKey); i++)
	...
	{
		if(i<szRandKey[0])
			bTempCode	= pszSrcData[i]^szRandKey[i%ENCRYPT_KEY_LEN];
		else
			bTempCode	= szRandKey[i%ENCRYPT_KEY_LEN]^(rand()%255);
...
WORD CXOREncrypt::CrevasseData(LPCTSTR pszEncrypData, LPTSTR pszSrcData, WORD wSize)
{
	...
	if(wEncrypPassLen<ENCRYPT_KEY_LEN*4)
		return	0;
	...
	for(int i=0; i<wSoureLength; i++)
	{
		...
		szTempBuffer[2]	= 0;
		uKey			= strtol((LPCTSTR)szKeyBuffer, &pEnd, 16);
		uEncrypt		= strtol(szTempBuffer, &pEnd, 16);
