...
{
	//释放用户数组
	for(int i=0; i<CClientUserManager::m_UserItemStorage.GetCount(); i++)
		delete	CClientUserManager::m_UserItemStorage[i];
...
int CGameFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_BrowerAD.Navigate(TEXT("http://www.21898.com/AD/GamePlazaAD.asp"),NULL,NULL,NULL,NULL);
...
{
	m_DlgControlBar.WebBrowse(TEXT("http://www.21898.com/"),true);
	return;
}
...
{
	m_DlgControlBar.WebBrowse(TEXT("http://www.21898.com/Login.asp"),true);
	return;
}
...
{
	m_DlgControlBar.WebBrowse(TEXT("http://www.21898.com/pay.asp"),true);
	return;
}
...
{
	m_DlgControlBar.WebBrowse(TEXT("http://www.21898.com/ServeWealth.Asp"),true);
	return;
}
