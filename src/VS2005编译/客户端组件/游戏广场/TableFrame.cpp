...
{
	if(wChairID>=CountArray(m_TableState.pIUserItem))
		return	false;
...
bool __cdecl CTableFrame::InitTableFrame(WORD wTableCount, WORD wChairCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx)
{
	...
	//文件加载
	if(m_TableResource.LoadFromFiles(szDirectory)==false)
...
void CTableFrame::OnPaint()
{
	...
	if(m_pTableViewArray!=NULL)
	{
		...
		dc.FillSolidRect(&ClipRect, m_TableResource.m_crBackGround);
