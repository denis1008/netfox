	...
	case ItemGenre_Inside:
		{
			...
			WebBrowse(TEXT("http://www.21898.com"),false);
	...
	case ItemGenre_Kind:		//游戏类型
		{
			...
			_snprintf(szRuleUrl,sizeof(szRuleUrl),TEXT("http://game.21898.com/GameRule.aspx?KindID=%ld"),pGameKind->wKindID);
...
//左键双击
void __cdecl CPlazaViewItem::OnTreeLeftDBClick(CListItem *pListItem, HTREEITEM hTreeItem, CTreeCtrl *pTreeCtrl)
{
	//效验参数
	if(pListItem==NULL)
		return;
	...
	case ItemGenre_Process:	
		{
			...
			strCommonLine.Format(TEXT("%s /UserToken:%ld#%s#%s /KindID:%ld /WndPlaza:%ld"), pGameProcess->szProcessName, UserData.dwUserID, UserData.szAccounts, UserData.szPassWord, pGameProcess->wKindID, m_hWnd);
...
//选择改变
void __cdecl CPlazaViewItem::OnTreeSelectChanged(CListItem *pListItem, HTREEITEM hTreeItem, CTreeCtrl *pTreeCtrl)
{
	return;
}
...
{
	m_pHtmlBrower->Navigate("http://www.21898.com");
}
