...
{
	return	m_ClientSocket->SendData(MDM_GR_USER, SUB_GR_USER_STANDUP_REQ);
}

//发送强退命令
bool CRoomViewItem::SendLeftGamePacket()
{
	return	m_ClientSocket->SendData(MDM_GR_USER, SUB_GR_USER_LEFT_GAME_REQ);
}
...
bool CRoomViewItem::PerformSitDownAction(WORD wTableID, WORD wChairID)
{
	...
	//动作效验
	if(CanSitDownTable(wTableID, wChairID, false, true)==false)
		return	false;
...
//网络读取消息
bool __cdecl CRoomViewItem::OnSocketRead(CMD_Command Command, void* pBuffer, WORD wDataSize, IClientSocket* pIClientSocke)
{
...
bool CRoomViewItem::OnSocketSubStatus(CMD_Command Command, void* pBuffer, WORD wDataSize, IClientSocket* pIClientSocke)
{
	...
	WORD			wNowTableID		= pUserStatus->wTableID,
					wLastTableID	= pUserData->wTableID;
	WORD			wNowChairID		= pUserStatus->wChairID,
					wLastChairID	= pUserData->wChairID;
	BYTE			cbNowStatus		= pUserStatus->cbUserStatus,
					cbLastStatus	= pUserData->cbUserStatus;
	...
	{
		ASSERT(wNowChairID!=INVALID_CHAIR);
		if ( m_pListServer->GetItemInfo()->wKindID!=14 )
			m_TableFrame.UpdateTableView(wNowTableID,false);
		else
			m_TableFrame.UpdateTableView(wNowTableID,true);
	}
...
void __cdecl CRoomViewItem::OnLButtonHitTable(WORD wTableID, WORD wChairID, bool bMirror)
{
	...
	if(pTableUserItem!=NULL)
	{
		...
		{
			tagUserData*	pUserData	= m_pMeUserItem->GetUserData();
			if((pUserData->wTableID==wTableID)&&(pUserData->wChairID==wChairID)) 
			{
				return;
			}
