...
//菜单命令
#define	IDM_UM_FLOWER			WM_USER+100								//赠送礼物
...
VOID CVideoServiceControl::OnBnClickedFlower()
{
	...
	//菜单命令
	UINT uCommands[GIFT_COUNT] ;
	for ( WORD wCmdIdx = 0; wCmdIdx < GIFT_COUNT; ++wCmdIdx )
		uCommands[wCmdIdx] = IDM_UM_FLOWER + wCmdIdx;

	//菜单位图
	CBitmap MenuBmps[GIFT_COUNT];

	//加载位图
	MenuBmps[FLOWER_1].LoadBitmap( IDB_GIFT_RED_ROSE );
	MenuBmps[FLOWER_2].LoadBitmap( IDB_GIFT_SMALL_ROSE );
	MenuBmps[FLOWER_3].LoadBitmap( IDB_GIFT_LILY );
	...
	//设置菜单
	for ( UINT uMenuItem = 0; uMenuItem < GIFT_COUNT; ++uMenuItem )
	{
		//菜单信息
		CString strMenuItem = g_GiftInfo[uMenuItem].szName;
		CString strGiftPay;
		strGiftPay.Format("%ld", g_GiftInfo[uMenuItem].lValue);
	...
	//释放资源
	for ( INT nBmpIdx = 0; nBmpIdx < GIFT_COUNT; ++nBmpIdx ) MenuBmps[nBmpIdx].DeleteObject();
...
BOOL CVideoServiceControl::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if ( IDM_UM_FLOWER <= uCmd && uCmd <= IDM_UM_FLOWER + GIFT_COUNT )
	{
		//索引转换
		...
		//构造信息
		CString strMsg;
		strMsg.Format(TEXT("你选择了送(%s%ld)，将会被扣除[%ld]游戏币\n按“是”继续\n按“否”取消这次操作"),g_GiftInfo[uFlowerIdx].szName, g_GiftInfo[uFlowerIdx].lValue, g_GiftInfo[uFlowerIdx].lValue );
		int iRet=AfxMessageBox(strMsg,MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2);
		if (iRet!=IDYES) return TRUE;

		//存在判断
		DWORD dwRcvUserID = GetVideoUserID();
		if ( dwRcvUserID == 0 || m_dwTargerUserID != dwRcvUserID )
		{
			AfxMessageBox( TEXT("对方已经离开，赠送失败！"), MB_OK | MB_ICONINFORMATION );
			return TRUE;
		}
		...
		//发送消息
		pIClientKernel->SendSocketData( MDM_GF_FRAME,SUB_GF_GIFT,&Gift, sizeof(Gift));
