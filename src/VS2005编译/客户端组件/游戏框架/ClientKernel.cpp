...
bool __cdecl CClientKernel::SetMessageProxy(IUnknownEx * pIUnknownEx)
{
	...
	if (pVideoServiceManager!=NULL) pVideoServiceManager->SetMessageProxy(GET_OBJECTPTR_INTERFACE(m_pIMessageProxy,IUnknownEx));
				...
				case SUB_GF_GIFT:			//赠送鲜花
					{
						...
						ASSERT( pGiftNotify->wGiftID < GIFT_COUNT ) ;
						if ( pGiftNotify->wGiftID >= GIFT_COUNT ) return true;

						//构造消息
						CString strGiftMsg;
						strGiftMsg.Format( g_GiftResInfo[pGiftNotify->wGiftID].szShowMsg, pSendUserData->szName, pRcvUserData->szName, 
							g_GiftInfo[pGiftNotify->wGiftID].lValue, pSendUserData->szName, pRcvUserData->szName );

						//显示消息
						m_pIMessageProxy->InsertSystemString(strGiftMsg,MS_NORMAL);
...
void CClientKernel::UpdateGameTitle()
{
	...
	//构造标题
	if ((m_ServerAttribute.szServerName[0]!=0)&&(m_wTableID!=INVALID_CHAIR))
	{
